...
# customizing stopwords taken from NLTK
custom_stopwords = {
...
# keeping only rows where word is not a stopword
df = df[~df['1-gram'].isin(custom_stopwords)]

# finding most common 5 words in dataframe and extracting frequency per date
top_5_grams = df.groupby('1-gram')['count'].sum().nlargest(5).index.tolist()
df_top5 = df[df['1-gram'].isin(top_5_grams)].copy()
print(df_top5)

# for time-series plotting, now adding date column 
...
# saving as HTML
fig.write_html("Sara_Baig_top5_evolutions.html")
