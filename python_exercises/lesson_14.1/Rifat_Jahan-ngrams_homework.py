...
# Combining year, month, day into a single datetime column
...
# Filtering dataframe to keep only top 5 words
df_top = df[df["1-gram"].isin(top_words)].copy()
print(df_top.head())

# Creating a "month" column for grouping
df_top["month"] = df_top["date"].dt.to_period("M").dt.to_timestamp()

# Grouping by month and unigram, sum counts
