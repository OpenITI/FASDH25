...
#looking for the colmun with longest length and returing the max value in that column
max_length= df["length"].max()

#filtering: selecting only rows where lenghth is equal to max lenghth
longest_title=df[df["length"]== max_length]

#get the title column from filtered row                 
...
#sort all the dataframe based on values in the length column
top_20= df.sort_values(by='length', ascending=False).head(20)


#exporting the top 20 longest artcile to a csv
top_20.to_csv('outputs/Bushra-Hoorani-top20.csv', index=False)
