...
# Remove unclassified articles(articles with no topic assigned
df = df[df["Topic"] != -1]

# Create a readable label for topics
df["Topic_Label"] = df[["topic_1", "topic_2", "topic_3", "topic_4"]].agg(", ".join, axis=1)

# Group by year and topic( counts how many articles are in each year-topic combination)
...
# Save to HTML
fig.write_html("Jafar_Top5_Topics_by_Year.html")
