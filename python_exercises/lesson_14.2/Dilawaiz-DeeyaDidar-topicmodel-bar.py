...
#print head to understand structure of the data
print(df.head())

# filter rows where Topic is -1 
df = df[df["Topic"] != -1]

# Create a single label by combining topic columns with commas
df["Topic_Label"] = df[["topic_1", "topic_2", "topic_3", "topic_4"]].agg(", ".join, axis=1)

# Group the DataFrame by Topic_Label and year, count the number of articles belonging to every topic in each year,
...
# write bar chart as html
fig.write_html("Dilawaiz_Article_Counts_by_Year_and_Topic.html")
