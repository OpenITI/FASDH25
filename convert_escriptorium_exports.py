"""This script converts all files in a folder of
eScriptorium exports into OpenITI mARkdown format.

It is based on the add_escriptorium_texts.py in barzakh.
...
from openiti.helper.yml import readYML, dicToYML, ymlToDic
...
escript_message = """This text was OCR'ed as part of the
Text-Based Digital Humanities course at the
Faculty of Arts and Sciences, Aga Khan University, Karachi,
in Spring 2025. The goal was to learn to train an OCR model.
OCR output will not be perfect."""
...
def get_tsv_metadata(meta_fp, first_row=1, last_row=1000, coll_id="FASDH",
                     default_transcription_layer="FASDH25_transcription"):
    """get metadata from a tsv file
    ...
    with open(meta_fp, mode="r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            if not layer:
                print("!!!!!!NO TRANSCRIPTION LAYER!!!!!!")
                layer = default_transcription_layer

            timestamp = datetime.utcnow().strftime("%y%m%d")
            version_id = f"{coll_id}0{timestamp}{i:02d}-urd1"
            ...
            if author_transcr:
                yml_d = ymlToDic(author_yml_template)
                yml_d["10#AUTH#SHUHRA#AR:"] = author_transcr
                author_yml = dicToYML(yml_d)
            else:
                author_yml = ""
                print('!!!!!!!NO AUTHOR TRANSCRIPTION!!!!!!!')
            ...
            if row["NOTES"]:
                notes += "¶    " + row["NOTES"]
            version_yml = add_to_yml(None, based, link, notes, issues, uri="")
...
##                    file.write(book_yml_template)
                yml_d = ymlToDic(book_yml_template)
                yml_d["10#BOOK#TITLEA#AR:"] = title_transcr
                book_yml = dicToYML(yml_d)
            else:
                book_yml = ""
                print('!!!!!!!NO TITLE TRANSCRIPTION!!!!!!!')
            ...
            tsv_d[key] = {
                ...
                "book_yml": book_yml,
                "author_yml": author_yml,
                "version_id": version_id,
                "eScr_project": project,
                "eScr_doc": doc,
                "transcr_layer": layer
            }
            print(json.dumps(tsv_d[key]))
    return tsv_d
...
def convert_folder_of_zips(zips_folder, outfolder, tsv_d, include_regions=[],
                           exclude_regions=[], page_offset=0,
    ...
    Args:
        zips_folder (str): path to the folder that contains the zip files
        include_regions (list): list of names of region types from which the
            text should be extracted
        exclude_regions (list): list of names of region types from which the
            text should NOT be extracted
    ...
    """
    for fn in os.listdir(zips_folder):
        student_folder = os.path.join(zips_folder, fn)
        for f in os.listdir(student_folder):
            if f.endswith(".zip"):
                print(f)
                zip_fp = os.path.join(student_folder, f)
                # get metadata from the zip filename
                try:
                    fn_data = f.replace("export_doc", "")
                    fn_data = re.sub("_pagexml.+", "", fn_data)
                    doc_pk = re.findall("^\d+", fn_data)[0]
                    group_name = re.sub(doc_pk+"_", "", fn_data)
                    transcription_meta["transcribers"] = group_name
                    transcription_meta["Escriptorium document pk"] = doc_pk
                    print("GROUP NAME:", group_name)
                except:
                    group_name = fn
                    fn_data = fn
                group_folder = os.path.join(outfolder, group_name)
                if not os.path.exists(group_folder):
                    os.makedirs(group_folder)
                outfp =  os.path.join(group_folder, fn_data)
                if os.path.exists(outfp) and not overwrite:
                    print(outfp, "already exists. Not converting!")
                    continue
                ...
                try:
                    ...
                    with open(outfp+"_book.yml", mode="w", encoding="utf-8") as file:
                        file.write(tsv_d[key]["book_yml"])
                    with open(outfp+"_version.yml", mode="w", encoding="utf-8") as file:
                        file.write(tsv_d[key]["version_yml"])
                except:
                    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    print("GROUP", key, "NOT IN TSV")
                    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
if __name__ == "__main__":
   meta_fp = "meta/FASDH25_OCR_meta - ESCRIPTORIUM.tsv"

   ignore = {
        #"Ammara Karim",   # daughters_of_night
        #"Mahpara Karim",  # daughters_of_night
        #"Bushra Hoorani", # daughters_of_night
        "Yasir Rauf",     # middle_ground
        "Kulsoom Zaman",  # middle_ground
        "Abdus Salam",    # programmer
   ...
   transcription_meta = {
       ...
       "segmentation model": "urdu_segm_fas25.mlmodel"
       }
